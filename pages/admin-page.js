...
export class AdminPage {
  constructor(page) {
    ...
    this.lunchEditBlock = page.locator('a.v-list__tile:has-text("Lunch Editing")');
    this.buildButton = page.locator('.v-btn__content >> .v-icon:has-text("build")');
    this.addProviderButton = page.locator('.v-btn__content >> .v-icon:has-text("add")');
    this.providerNameField = page.locator('input[aria-label="Provider Name"][name="Provider Name"][role="combobox"]');
    this.providerColorField = page.locator('input[aria-label="Color"]');
    this.addDishButton = page.locator('button:has-text("Dish")').nth(1);
    this.selectionNameField =  page.locator('input[aria-label="Selection Name"][name="Sriubos (Soups) category"]');
    this.priceField = page.locator('input[aria-label="Price"][name="Sriubos (Soups) category"]');
    this.countField = page.locator('input[aria-label="Count"][name="Sriubos (Soups) category"]');
    this.translationField = page.locator('.v-text-field__slot >> input[aria-label="Translation"]');
    this.saveButton = page.locator('button.v-btn--flat.theme--dark:has-text("Save")');
  ...
  async validateAdminLogin() {
    await expect(this.nameLocator).toHaveText(testData.loggedInAdminUser, { timeout: 5000 });
  }
